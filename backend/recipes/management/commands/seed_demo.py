    ...
    def handle(self, *args, **options):
        ...
        for user, _ in created_users:
            ...
            if Recipe.objects.filter(author=user).exists():
                self.stdout.write(
                    f'{user.email} already has recipes, skipping'
                )
                continue

            recipe_name = (
                f'Рецепт от {user.first_name or user.username}'
            )
            recipe_text = (
                'Описание шага 1. Описание шага 2. Приятного аппетита!'
            )
            ...
            # Tags
            recipe.tags.set(
                random.sample(all_tags, k=min(2, len(all_tags)))
            )

            # Ingredients
            for ing in random.sample(
                all_ingredients,
                k=min(3, len(all_ingredients)),
            ):
                RecipeIngredient.objects.create(
                    recipe=recipe,
                    ingredient=ing,
                    amount=random.randint(1, 5) * 50,
                )

            self.stdout.write(
                self.style.SUCCESS(
                    f'Created recipe "{recipe.name}" for {user.email}'
                )
            )

        self.stdout.write(
            self.style.SUCCESS('Demo data seeding complete.')
        )
